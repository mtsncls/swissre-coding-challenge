...
public class OrgChartBuilder {
    ...
    private Employee findCeo(List<Employee> employees) {
        return employees.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("No CEO found (employee without managerId)."));
    }
    ...
    private void attachToManager(Employee e, Map<Integer, Employee> index) {
        ...
        if (manager == null) {
            throw new IllegalStateException("Manager with ID " + e.getManagerId() + " not found for employee " + e.getId());
        }
